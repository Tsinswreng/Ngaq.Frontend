...
public partial class VmWeightCalculatorQuery: ViewModelBase, IMk<VmWeightCalculatorQuery>{
	protected VmWeightCalculatorQuery(){ StudyPlanUiStore.EnsureInit(); Refresh(); }
	public static VmWeightCalculatorQuery Mk()=>new();
	...
	public ObservableCollection<UiWeightCalculator> PageItems{ get=>field; set=>SetProperty(ref field,value);}=[];
	...
	public nil Refresh(){
		IEnumerable<UiWeightCalculator> seq = StudyPlanUiStore.WeightCalculators;
		...
		var list = seq.ToList();
		Total = list.Count;
		PageItems = new ObservableCollection<UiWeightCalculator>(list.Skip(PageIdx*PageSize).Take(PageSize));
		OnPropertyChanged(nameof(PageText));
		return NIL;
	}
}

public partial class VmWeightCalculatorEdit: ViewModelBase, IMk<VmWeightCalculatorEdit>{
	protected VmWeightCalculatorEdit(){ StudyPlanUiStore.EnsureInit(); }
	public static VmWeightCalculatorEdit Mk()=>new();
	...
	public nil Load(UiWeightCalculator? src){
		if(src is null){
	...
	public nil Save(){
		...
		var got = StudyPlanUiStore.WeightCalculators.FirstOrDefault(x=>x.Id == Id);
		if(got is null){
			StudyPlanUiStore.WeightCalculators.Add(new UiWeightCalculator{ Id = Id.Trim(), UniqName = UniqName.Trim(), Descr = Descr, JsonText = JsonText });
		}else{
			got.UniqName = UniqName.Trim();
			got.Descr = Descr;
			got.JsonText = JsonText;
		}
		ShowMsg("WeightCalculator 已保存");
		return NIL;
	}
	...
	public nil Delete(){
		var got = StudyPlanUiStore.WeightCalculators.FirstOrDefault(x=>x.Id == Id);
		if(got is not null){
			StudyPlanUiStore.WeightCalculators.Remove(got);
			ShowMsg("WeightCalculator 已刪除");
		}
		return NIL;
	}
}

public partial class ViewWeightCalculatorQuery: AppViewBase{
	public VmWeightCalculatorQuery? Ctx{ get=>DataContext as VmWeightCalculatorQuery; set=>DataContext = value; }
	public ViewWeightCalculatorQuery(){ Ctx = VmWeightCalculatorQuery.Mk(); Render(); }
	...
	Control MkTop(){
		...
		g.AddInit(new TextBox{ Watermark = "Search WeightCalculator" }, o=>o.Bind(o.PropText, CBE.Mk<VmWeightCalculatorQuery>(x=>x.SearchText, Mode:BindingMode.TwoWay)));
	...
	Control MkList(){
		...
		items.Bind(items.PropItemsSource, CBE.Mk<VmWeightCalculatorQuery>(x=>x.PageItems));
		items.SetItemsPanel(()=>new StackPanel{ Spacing = 6 });
		items.SetItemTemplate<UiWeightCalculator>((row, ns)=>{
	...
	Control MkPageBar(){
		...
		g.AddInit(new TextBlock{ VerticalAlignment = VAlign.Center }, o=>o.Bind(o.PropText, CBE.Mk<VmWeightCalculatorQuery>(x=>x.PageText)));
		g.AddInit(new Button{ Content=">", MinHeight = 34 }, o=>o.Click+=(s,e)=>Ctx?.Next());
		return g.Grid;
	}

	void OpenEdit(UiWeightCalculator? row){
		var v = new ViewWeightCalculatorEdit();
		v.Ctx?.Load(row);
		Ctx?.ViewNavi?.GoTo(ToolView.WithTitle("WeightCalculator Edit", v));
	}
}

public partial class ViewWeightCalculatorEdit: AppViewBase{
	public VmWeightCalculatorEdit? Ctx{ get=>DataContext as VmWeightCalculatorEdit; set=>DataContext = value; }
	public ViewWeightCalculatorEdit(){ Ctx = VmWeightCalculatorEdit.Mk(); Render(); }
	...
	Control MkForm(){
		...
		sp.Children.Add(MkInput("Id", CBE.Mk<VmWeightCalculatorEdit>(x=>x.Id, Mode:BindingMode.TwoWay), true));
		sp.Children.Add(MkInput("UniqName", CBE.Mk<VmWeightCalculatorEdit>(x=>x.UniqName, Mode:BindingMode.TwoWay)));
		sp.Children.Add(MkInput("Descr", CBE.Mk<VmWeightCalculatorEdit>(x=>x.Descr, Mode:BindingMode.TwoWay)));
		var tb = new TextBox{ AcceptsReturn = true, Height = 240, TextWrapping = TextWrapping.Wrap };
		tb.Bind(tb.PropText, CBE.Mk<VmWeightCalculatorEdit>(x=>x.JsonText, Mode:BindingMode.TwoWay));
